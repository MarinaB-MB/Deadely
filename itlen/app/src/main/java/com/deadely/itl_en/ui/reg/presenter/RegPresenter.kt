...
import com.deadely.itl_en.network.ErrorResponse
...
class RegPresenter @Inject constructor(private var db: AppDatabase, private var apiInterface: IRestDBService) : BasePresenter<IRegContract.View>(), IRegContract.Presenter {
        ...
        call.enqueue(object : retrofit2.Callback<User> {
            override fun onResponse(call: Call<User>, response: Response<User>) {
                if (response.isSuccessful) {
                    ...
                    if (errorResponse.list[0].message.contains("Already exists")) {
                        getMvpView()?.showEmailError(FieldConverter().getString(R.string.user_already_exist))
                    } else {
    ...
    override fun clearActiveUser() {
        if (ud.getActiveUser(true) != null) {
            val activeUser = ud.getActiveUser(true)
