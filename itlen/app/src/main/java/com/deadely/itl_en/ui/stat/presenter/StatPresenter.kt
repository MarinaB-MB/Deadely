...
class StatPresenter @Inject constructor(private var db: AppDatabase, private var apiInterface: IRestDBService) : BasePresenter<IStatContract.View>(), IStatContract.Presenter {
    ...
    private lateinit var list: List<Stat>
    ...
    override fun getUserStat() {
        if (ud.getActiveUser(true).stats != null) {
            list = ud.getActiveUser(true).stats
            val lastStat = list.last()
            getMvpView()?.initData(lastStat)
        }
